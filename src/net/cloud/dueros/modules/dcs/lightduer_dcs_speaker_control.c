...
error_out:
    duer_ds_log_dcs_event_report_fail("VolumeState");
...
void duer_dcs_speaker_control_init(void)
{
    ...
    duer_add_dcs_directive(res, sizeof(res) / sizeof(res[0]), speaker_control_namespace);
    duer_reg_client_context_cb(duer_get_speak_control_state_internal);
}
