...
static baidu_json *report_system_dynamic_info(void)
{
    ...
    if (dynamic_system_info.system_load != NULL) {
        ...
        if (sys_load != NULL) {
            memset(sys_load, 0, dynamic_system_info.system_load_len + 1);

            strncpy(sys_load, dynamic_system_info.system_load, dynamic_system_info.system_load_len);
    ...
    if (dynamic_system_info.task_info != NULL) {
        ...
        while (element) {
            ...
            baidu_json_AddStringToObject(task_info_json, "Name", element->task_name);

            baidu_json_AddNumberToObject(task_info_json, "Stack", element->stack_size);
            baidu_json_AddNumberToObject(task_info_json, "Free_Stack", element->stack_free_size);

            if (element->cpu_occupancy_rate >= 0) {
                baidu_json_AddNumberToObject(task_info_json, "CPU", element->cpu_occupancy_rate);
            }
    ...
    baidu_json_AddItemToObject(system_dynamic_info_json, "Task_Info", task_array_json);
...
static baidu_json *report_memory_info(void)
{
    ...
    case RAM:
        baidu_json_AddStringToObject(memory_info_json, "Type", "RAM");
        break;
    case PSRAM:
        baidu_json_AddStringToObject(memory_info_json, "Type", "PSRAM");
        break;
    default:
...
static baidu_json *report_disk_info(void)
{
    ...
    while (disk_info) {
        ...
        baidu_json_AddStringToObject(disk_info_json, "Name", disk_info->partition_name);
        if (strlen(disk_info->mount_info) != 0) {
            baidu_json_AddStringToObject(disk_info_json, "Mount_Info", disk_info->mount_info);
        }
        baidu_json_AddNumberToObject(disk_info_json, "Size", disk_info->total_size);
        baidu_json_AddNumberToObject(disk_info_json, "Used", disk_info->used_size);
        baidu_json_AddNumberToObject(disk_info_json, "Available", disk_info->available_size);
        baidu_json_AddNumberToObject(disk_info_json, "Unilization", disk_info->unilization);
    ...
    baidu_json_AddItemToObject(report_data_json, "Disk", disk_info_array_json);
...
static baidu_json *report_network_info(void)
{
    ...
    if (strlen(network_info.hw_address) != 0) {
        baidu_json_AddStringToObject(
                network_info_json,
                "HW_Addr",
                network_info.hw_address);
    }
    if (strlen(network_info.ipv4_address) != 0) {
        baidu_json_AddStringToObject(
                network_info_json,
                "IPV4_Addr",
                network_info.ipv4_address);
    }
    if (strlen(network_info.ipv6_address) != 0) {
        baidu_json_AddStringToObject(
                network_info_json,
                "IPV6_Addr",
...
void duer_system_info_show_system_dynamic_info(duer_system_dynamic_info_t const *info)
{
    ...
    if (info->system_load != NULL) {
        ...
        if (sys_load != NULL) {
            memset(sys_load, 0, info->system_load_len + 1);

            strncpy(sys_load, info->system_load, info->system_load_len);
