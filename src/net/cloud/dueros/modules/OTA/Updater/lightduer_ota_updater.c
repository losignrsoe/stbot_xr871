...
static duer_ota_init_ops_t *s_ota_init_ops = NULL;
...
static int notify_package_info(duer_ota_updater_t *updater)
{
    int ret = DUER_OK;
...
static int handler(void *private, const char *buf, size_t len)
{
    ...
    if (buf != NULL && len > 0) {
        ...
        ret = duer_ota_unpack_decompress_data(updater->unpacker, (unsigned char *)data, len);
        if (ret != DUER_OK) {
...
static int duer_ota_create_updater(OTA_Updater ota_updater, duer_ota_update_command_t *update_cmd)
{
    int ret = DUER_OK;
    int tmp = DUER_OK;
...
static duer_downloader_protocol get_download_protocol(char *url, size_t len)
{
    /*
...
static void ota_updater(int arg, void *ota_updater)
{
    ...
    ret = duer_ota_unpack_verification(unpacker->verification);
    if (ret != DUER_OK) {
        DUER_LOGE("OTA Updater: Verify the updating package failed ret: %d", ret);
...
static int duer_analyze_update_cmd(const char *update_cmd, size_t cmd_len, duer_ota_update_command_t *ota_update_cmd)
{
    int ret = DUER_OK;
    char *cmd_str = NULL;
    baidu_json *cmd = NULL;
...
int duer_init_ota(duer_ota_init_ops_t *ops)
{
    int ret = DUER_OK;
