...
static duer_dcache_item *duer_dcache_item_create(const void *data, size_t size) {
    duer_dcache_item *cache = (duer_dcache_item *)DUER_MALLOC(sizeof(duer_dcache_item));
...
duer_status_t duer_dcache_push(const void *data, size_t size) {
    ...
    do {
        ...
        cache = duer_dcache_item_create(data, size);
        if (cache == NULL) {
