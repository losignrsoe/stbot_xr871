...
static volatile int s_split_id;
static duer_mutex_t s_split_mute;
#endif
...
{
    if (!s_split_mute) {
        s_split_mute = duer_mutex_create();
    }

    duer_mutex_lock(s_split_mute);
    s_split_id = duer_random();
    duer_mutex_unlock(s_split_mute);
}

static void duer_split_id_finalize()
{
    if (s_split_mute) {
        duer_mutex_destroy(s_split_mute);
        s_split_mute = NULL;
    }
}
#endif
...
void duer_initialize()
{
    ...
    duer_ds_register_report_function(duer_ds_report_ca_status);
...
int duer_data_report_async(duer_context_t *context, const baidu_json *data)
{
    ...
    do {
        ...
        DUER_LOGD("split send data");

        buf = DUER_MALLOC(MAX_PAYLOAD_LEN + 1);
        if (!buf) {
            rs = DUER_ERR_MEMORY_OVERLOW;
            break;
        }
        ...
        baidu_json_AddItemToObject(value, "duer_blockwise", payload);
        baidu_json_AddNumberToObject(payload, "id", id);
        baidu_json_AddNumberToObject(payload, "eof", 0);
        remain_len = content_len;

        while (remain_len > 0) {
            baidu_json_DeleteItemFromObject(payload, "block");
            baidu_json_AddNumberToObject(payload, "block", block++);
            ...
            if (remain_len == 0) {
                ...
                baidu_json_AddNumberToObject(payload, "eof", 1);
            }
            baidu_json_DeleteItemFromObject(payload, "block_data");
            baidu_json_AddStringToObject(payload, "block_data", buf);
