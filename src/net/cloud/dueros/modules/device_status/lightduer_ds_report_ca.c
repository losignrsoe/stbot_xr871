...
baidu_json* duer_ds_report_ca_status(void)
{
    ...
    baidu_json_AddNumberToObject(send, "avg_latency", s_ca_status.avg_latency);
    baidu_json_AddNumberToObject(send, "max_packet", s_ca_status.s_max_packet);

    baidu_json_AddItemToObject(content, "send", send);
    ...
    baidu_json_AddNumberToObject(recv, "valid_recv", s_ca_status.valid_recv);
    baidu_json_AddNumberToObject(recv, "recv_count", s_ca_status.recv_count);
    baidu_json_AddNumberToObject(recv, "max_packet", s_ca_status.r_max_packet);
    ...
    if (s_ca_status.mbedtls
        ...
        if (s_ca_status.mbedtls) {
            baidu_json_AddNumberToObject(error, "mbedtls", s_ca_status.mbedtls);
        }
        if (s_ca_status.tcp_header) {
            baidu_json_AddNumberToObject(error, "tcp_header", s_ca_status.tcp_header);
        }
        if (s_ca_status.coap) {
            baidu_json_AddNumberToObject(error, "coap", s_ca_status.coap);
        }

        baidu_json_AddItemToObject(content, "error", error);
    }

exit:
    if (content) {
        baidu_json_AddItemToObject(status, "ca_status", content);
    }
    return status;
}
