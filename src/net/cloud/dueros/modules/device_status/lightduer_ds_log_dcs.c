...
duer_status_t duer_ds_log_dcs_directive_drop(int current_id, int directive_id, const char *name)
{
    ...
    baidu_json_AddNumberToObject(msg, "current_dialog_id", current_id);
    baidu_json_AddNumberToObject(msg, "directive_dialog_id", directive_id);
    if (name) {
        baidu_json_AddStringToObject(msg, "directive", name);
    } else {
        baidu_json_AddStringToObject(msg, "directive", "");
    }
    ret = duer_ds_log_dcs(DUER_DS_LOG_DCS_DIRECTIVE_DROPPED, msg);
