...
static duer_status_t duer_ds_log_set_level(duer_context ctx, duer_msg_t* msg, duer_addr_t* addr)
{
    ...
    if (handler && msg) {
        ...
        if (msg->payload && msg->payload_len > 0) {
            char *json = (char*)DUER_MALLOC(msg->payload_len + 1);

            if (json) {
                memcpy(json, msg->payload, msg->payload_len);
                json[msg->payload_len] = '\0';
                DUER_LOGD("duer_ds_log_set_level, cjson:%s", json);
                duer_ds_log_level_enum_t report_level = DUER_DS_LOG_DEFAULT_REPORT_LEVEL;

                baidu_json *value = baidu_json_Parse(json);
                if (value) {
                              ...
                              const baidu_json *log_message)
{
    ...
    if (log_message) {
        baidu_json_AddItemToObject(content, "message", (baidu_json*)log_message);
    }
    ...
    baidu_json_AddNumberToObject(content, "code", code);
    timestamp = duer_timestamp();
    baidu_json_AddNumberToObject(content, "ts", timestamp);

    baidu_json_AddItemToObject(log, "duer_trace_info", content);
#ifdef LIGHTDUER_DS_LOG_DEBUG
