...
duer_status_t duer_ds_log_rec_stop(duer_u32_t id) {
    ...
    do {
        ...
        for (i = 0; i < DUER_REC_STAT_ENUM_NUM; i++) {
            baidu_json_AddNumberToObject(infos[i], "max", g_statistics[i]._max);
            baidu_json_AddNumberToObject(infos[i], "min", g_statistics[i]._min);
            if (g_statistics[i]._counts) {
                baidu_json_AddNumberToObject(infos[i], "avg",
                        1.0f * g_statistics[i]._sum / g_statistics[i]._counts);
            }

            baidu_json_AddItemToObject(message, g_tags[i], infos[i]);
